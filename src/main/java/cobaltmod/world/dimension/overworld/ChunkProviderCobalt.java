...
import cobaltmod.world.gen.WorldGenDarkLakes;
import cpw.mods.fml.common.eventhandler.Event.Result;

public class ChunkProviderCobalt implements IChunkProvider
{
    /**
     * RNG.
     */
    ...
    public ChunkProviderCobalt(World par1World, long par2, boolean par4)
    {
                    ...
                    for (int l2 = 0; l2 < 8; ++l2)
                    {
                        ...
                        for (int i3 = 0; i3 < 4; ++i3)
                        {
                            ...
                            for (int k3 = 0; k3 < 4; ++k3)
                            {
                                if ((d15 += d16) > 0.0D)
                                {
                                    p_147424_3_[j3 += short1] = CMContent.corruptedstone;
                                }
                                else if (k2 * 8 + l2 < b0)
                                {
                                    p_147424_3_[j3 += short1] = CMContent.darkwater;
                                }
                                else
                                {
    ...
    public void replaceBlocksForBiome(int p_147422_1_, int p_147422_2_, Block[] p_147422_3_, byte[] p_147422_4_, BiomeGenBase[] p_147422_5_)
    {
        ...
        for (int a = 0; a < 16; ++a)
        {
            for (int b = 0; b < 16; ++b)
            {
                BiomeGenBase biomegenbase = p_147422_5_[b + a * 16];
                //this.genBiomeTerrainCobalt(this.worldObj, this.rand, p_147422_3_, p_147422_4_, p_147422_1_ * 16 + k, p_147422_2_ * 16 + l, this.stoneNoise[l + k * 16]);
                
                @SuppressWarnings("unused")
				boolean flag = true;
                Block block = CMContent.cobaltgrass;
                byte b0 = (byte)(biomegenbase.field_150604_aj & 255);
                Block block1 = CMContent.cobaltdirt;
                int k = -1;
                int l = (int)(this.stoneNoise[b + a * 16] / 3.0D + 3.0D + this.rand.nextDouble() * 0.25D);
                int i1 = p_147422_1_ * 16 + a & 15;
                int j1 = p_147422_2_ * 16 + b & 15;
                ...
                for (int l1 = 255; l1 >= 0; --l1)
                {
                    ...
                    {
                        Block block2 = p_147422_3_[i2];

                        if (block2 != null && block2.getMaterial() != Material.air)
                        {
                            if (block2 == CMContent.corruptedstone)
                            {
                                if (k == -1)
                                {
                                    if (l <= 0)
                                    {
                                        block = null;
                                        b0 = 0;
                                        block1 = CMContent.corruptedstone;
                                    }
                                    else if (l1 >= 59 && l1 <= 64)
                                    {
                                        block = CMContent.cobaltgrass;
                                    ...
                                    {
                                        if (biomegenbase.getFloatTemperature(p_147422_1_ * 16 + k, l1, p_147422_2_ * 16 + l1) < 0.15F)
                                        {
                                            block = Blocks.ice;
                                            b0 = 0;
                                        }
                                    ...
                                    else if (l1 < 56 - l)
                                    {
                                        block = null;
                                        block1 = CMContent.corruptedstone;
                                        p_147422_3_[i2] = Blocks.gravel;
                                    }
                                    else
                                    {
                                    	p_147422_3_[i2] = block1;
                                    }
                                }
                                else if (k > 0)
                                {
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
            ...
            && TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, LAKE))
        {
            ...
            (new WorldGenDarkLakes(CMContent.darkwater)).generate(this.worldObj, this.rand, k1, l1, i2);
        }
