...
public class BlockCobexTorch extends BlockTorch {
	...
	private boolean canPlaceTorchOn(World par1World, int par2, int par3,
		...
		if (World.doesBlockHaveSolidTopSurface(par1World, par2, par3, par4)) {
			...
			return (l != null && l.canPlaceTorchOnTop(par1World, par2, par3,
					par4));
		}
	}
	...
	public int onBlockPlaced(World par1World, int par2, int par3, int par4,
		...
		if (par5 == 5
				&& par1World.isSideSolid(par2 - 1, par3, par4, EAST, true)) {
			j1 = 1;
		}
	...
	protected boolean func_94397_d(World par1World, int par2, int par3,
		...
		if (this.dropTorchIfCantStay(par1World, par2, par3, par4)) {
			...
			if (!par1World.isSideSolid(par2 + 1, par3, par4, WEST, true)
					&& i1 == 2) {
				flag = true;
			}

			if (!par1World.isSideSolid(par2, par3, par4 - 1, SOUTH, true)
					&& i1 == 3) {
				flag = true;
			}

			if (!par1World.isSideSolid(par2, par3, par4 + 1, NORTH, true)
					&& i1 == 4) {
				flag = true;
			}
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		if (l == 1) {
			par1World.spawnParticle("smoke", d0 - d4, d1 + d3, d2, 0.0D, 0.0D,
					0.0D);
			par1World.spawnParticle("flame", d0 - d4, d1 + d3, d2, 0.0D, 0.0D,
					0.0D);
		} else if (l == 2) {
			par1World.spawnParticle("smoke", d0 + d4, d1 + d3, d2, 0.0D, 0.0D,
					0.0D);
			...
			par1World.spawnParticle("smoke", d0, d1 + d3, d2 - d4, 0.0D, 0.0D,
					0.0D);
			par1World.spawnParticle("flame", d0, d1 + d3, d2 - d4, 0.0D, 0.0D,
					0.0D);
		} else if (l == 4) {
			par1World.spawnParticle("smoke", d0, d1 + d3, d2 + d4, 0.0D, 0.0D,
					0.0D);
