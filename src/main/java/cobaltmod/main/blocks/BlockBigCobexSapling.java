...
import cobaltmod.world.gen.WorldGenBigCobaltTree;
...
public class BlockBigCobexSapling extends BlockBush implements IGrowable {
	...
	public void markOrGrowMarked(World par1World, int par2, int par3, int par4, Random par5Random) {
		...
		for(int bl = par2 - 1; bl <= par2 + 1; bl++)
		{
			for(int bl1 = par4 - 1; bl1 <= par4 + 1; bl1++)
			{
				if (par1World.getBlock(bl, par3, bl1) == CMContent.cobexlog)
					{
						//System.out.println("NO");
						//System.out.println(bl + " " + par3 + "" + bl1);
						return;
					}
			}
		}
	...
	public void growTree(World par1World, int par2, int par3, int par4, Random par5Random) {
		...
		if (var6 == 1) {
			var7 = new WorldGenBigCobaltTree(true, 11 + par5Random.nextInt(3), 0, 0, false);
		} else if (var6 == 2) {
			var7 = new WorldGenBigCobaltTree(true, 11 + par5Random.nextInt(3), 0, 0, false);
		} else if (var6 == 3) {
			for (var8 = 0; var8 >= -1; --var8) {
				for (var9 = 0; var9 >= -1; --var9) {
					if (this.isSameSapling(par1World, par2 + var8, par3, par4 + var9, 3)
						...
						var7 = new WorldGenBigCobaltTree(true, 4 + par5Random.nextInt(2), 0, 0, false);
						var10 = true;
						break;
					}
				}
		...
		if (var10) {
			...
			par1World.setBlock(par2 + var8 + 1, par3, par4 + var9 + 1, Blocks.air);
		} else {
		...
		if (!((WorldGenerator) var7).generate(par1World, par5Random, par2 + var8, par3, par4 + var9)) {
			if (var10) {
				...
				par1World.setBlock(par2 + var8 + 1, par3, par4 + var9 + 1, this);
			} else {
