...
public class BlockBigCobexLeaves extends BlockLeavesBase implements IShearable
{
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
			...
			if ((l & 8) != 0 && (l & 4) == 0) {
				...
				if (par1World.checkChunksExist(par2 - i1, par3 - i1, par4 - i1,
					...
					for (l1 = -b0; l1 <= b0; ++l1) {
						for (i2 = -b0; i2 <= b0; ++i2) {
							for (j2 = -b0; j2 <= b0; ++j2) {
								block = par1World.getBlock(par2 + l1,
										par3 + i2, par4 + j2);
		...
		if (!par1World.isRemote) {
			for (int l = 0; l < 100; ++l) {
				...
				if (par1World.getBlock(i1, j1, k1) == Blocks.leaves || par1World.getBlock(i1, j1, k1) == Blocks.leaves
						&& par1World.getBlockLightValue(i1, j1 + 1, k1) >= 4
						&& par1World.getBlockLightOpacity(i1, j1 + 1, k1) <= 2) {
					par1World.setBlock(i1, j1, k1, CMContent.bigcobexleaves);
				}
			}
			for (int l = 0; l < 100; ++l) {
				int i1 = par2 + par5Random.nextInt(3) - 1;
				...
				if (par1World.getBlock(i1, j1, k1) == Blocks.log || par1World.getBlock(i1, j1, k1) == Blocks.log2) {
					par1World.setBlock(i1, j1, k1, CMContent.cobexlog);
				}
			}
		}
	}
    ...
    {
        return Item.getItemFromBlock(CMContent.bigcobexsapling);
    }
